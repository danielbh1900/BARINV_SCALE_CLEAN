...
static void lvgl_port_task(void *arg)
{
    ...
    while (lvgl_port_ctx.running) {
        ...
        /* BARINV_SCALE_CLEAN patch: drop the prior `(1 == task_delay_ms)`
         * special-case that clamped a 1 ms LVGL deadline to task_max_sleep_ms
         * (500 ms). LVGL returning 1 means "come back in 1 ms" — honour it. */
...
static void lvgl_port_flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    /* BARINV_SCALE_CLEAN patch (Phase 1 TWDT fix):
     * For RGB panels the io_handle is the 3-wire SPI init bus, which never
     * transfers pixel data, so the on_color_trans_done event registered above
     * never fires and lv_disp_flush_ready is never called — LVGL's
     * draw_buf_flush then spins forever in `while (draw_buf->flushing)`,
     * holding lv_timer_handler and starving the IDLE task on whichever core
     * the LVGL task is pinned to (TWDT fires).
     * esp_lcd_panel_draw_bitmap for RGB panels is a synchronous memcpy into
     * the framebuffer — the moment it returns, the source buffer is free
     * for LVGL to reuse, so calling lv_disp_flush_ready here is correct.
     * (If an SPI panel is ever added to this project, this synchronous
     *  flush_ready would be wrong for SPI DMA and the on_color_trans_done
     *  path should be re-enabled instead.) */
